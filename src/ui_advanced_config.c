...
#define SCOPE_DATA(o)                                                          \
  E_Config_Dialog_Data* cfdata = evas_object_data_get(o, "__screen_settings");
...
static void
_cb_restore_changed(void* data EINA_UNUSED,
                    Evas_Object* obj,
                    void* event EINA_UNUSED)
{
  E_Config_Dialog_Data* cfdata = data;
  cfdata->restore = elm_check_state_get(obj);
  evas_object_smart_callback_call(cfdata->obj, "changed", NULL);
}

static void
_cb_hotplug_changed(void* data EINA_UNUSED,
                    Evas_Object* obj,
                    void* event EINA_UNUSED)
{
  E_Config_Dialog_Data* cfdata = data;
...
static void
_cb_acpi_changed(void* data EINA_UNUSED,
                 Evas_Object* obj,
                 void* event EINA_UNUSED)
{
  E_Config_Dialog_Data* cfdata = data;
                           ...
                           void* event_info EINA_UNUSED)
{
  ...
  evas_object_smart_callback_call(p->cfdata->obj, "changed", NULL);
}
...
_basic_profile_list_fill(E_Config_Dialog_Data* cfdata, const char* prof_sel)
{
  ...
  EINA_LIST_FOREACH(names, l, profile)
  {
    ...
    p->cfdata = cfdata;
    list_it = elm_list_item_append(
      list, label, ic, NULL, _profiles_list_selected_cb, p);
    elm_object_item_del_cb_set(list_it, _profiles_list_item_del_cb);
    if ((prof_sel) && (!strcmp(profile, prof_sel)))
      sel_it = list_it;
...
static void
_basic_screen_info_fill(E_Config_Dialog_Data* cfdata,
                        E_Config_Randr2_Screen* cs,
                        E_Randr2_Screen* s)
{
  char buf[100];
  ...
  EINA_LIST_FOREACH(s->info.modes, l, m)
  {
    ...
    if (mode_cbdata) {
      ...
      it = elm_list_item_append(
        cfdata->modes_obj, buf, NULL, NULL, _cb_mode_set, mode_cbdata);
  ...
  if (it_sel)
    elm_list_item_selected_set(it_sel, EINA_TRUE);
  ...
  if (s->info.can_rot_0) {
    ...
    if (rot_cbdata) {
      ...
      rot_cbdata->rot = 0;
      it = elm_list_item_append(
        cfdata->rotations_obj, "0", NULL, NULL, _cb_rot_set, rot_cbdata);
  ...
  if (s->info.can_rot_90) {
    ...
    if (rot_cbdata) {
      ...
      rot_cbdata->rot = 90;
      it = elm_list_item_append(
        cfdata->rotations_obj, "90", NULL, NULL, _cb_rot_set, rot_cbdata);
      cfdata->freelist = eina_list_append(cfdata->freelist, rot_cbdata);
      if (cs->rotation == 90)
        it_sel = it;
    }
  ...
  if (s->info.can_rot_180) {
    ...
    if (rot_cbdata) {
      ...
      rot_cbdata->rot = 180;
      it = elm_list_item_append(
        cfdata->rotations_obj, "180", NULL, NULL, _cb_rot_set, rot_cbdata);
      cfdata->freelist = eina_list_append(cfdata->freelist, rot_cbdata);
      if (cs->rotation == 180)
        it_sel = it;
    }
  ...
  if (s->info.can_rot_270) {
    ...
    if (rot_cbdata) {
      ...
      rot_cbdata->rot = 270;
      it = elm_list_item_append(
        cfdata->rotations_obj, "270", NULL, NULL, _cb_rot_set, rot_cbdata);
      cfdata->freelist = eina_list_append(cfdata->freelist, rot_cbdata);
      if (cs->rotation == 270)
        it_sel = it;
    }
...
static void
_cb_profile_enabled_changed(void* data,
                            Evas_Object* obj EINA_UNUSED,
                            void* event EINA_UNUSED)
{
  E_Config_Dialog_Data* cfdata = data;
...
static void
_cb_custom_scale_changed(void* data,
                         Evas_Object* obj EINA_UNUSED,
                         void* event EINA_UNUSED)
{
  E_Config_Dialog_Data* cfdata = data;
...
static void
_cb_scale_value_changed(void* data,
                        Evas_Object* obj EINA_UNUSED,
                        void* event EINA_UNUSED)
{
  E_Config_Dialog_Data* cfdata = data;
...
static void
_cb_rel_align_changed(void* data,
                      Evas_Object* obj EINA_UNUSED,
                      void* event EINA_UNUSED)
{
  E_Config_Dialog_Data* cfdata = data;
...
static void
_cb_priority_changed(void* data,
                     Evas_Object* obj EINA_UNUSED,
                     void* event EINA_UNUSED)
{
  E_Config_Dialog_Data* cfdata = data;
...
{
  char pbuf[128];
  const char* policy[] = { _("Ignore"), _("Extend"), _("Clone"), _("Ask") };

  snprintf(
    pbuf, sizeof(pbuf), _("Hotplug Policy (%s)"), policy[cfdata->policy]);
  elm_object_text_set(cfdata->policy_obj, pbuf);
}

static void
_cb_policy_ignore(void* data,
                  Evas_Object* obj EINA_UNUSED,
                  void* event_info EINA_UNUSED)
{
  E_Config_Dialog_Data* cfdata = data;

  if (cfdata->policy == E_RANDR2_POLICY_NONE)
    return;
...
static void
_cb_policy_ask(void* data,
               Evas_Object* obj EINA_UNUSED,
               void* event_info EINA_UNUSED)
{
  E_Config_Dialog_Data* cfdata = data;

  if (cfdata->policy == E_RANDR2_POLICY_ASK)
    return;
...
static void
_cb_policy_extend(void* data,
                  Evas_Object* obj EINA_UNUSED,
                  void* event_info EINA_UNUSED)
{
  E_Config_Dialog_Data* cfdata = data;

  if (cfdata->policy == E_RANDR2_POLICY_EXTEND)
    return;
...
static void
_cb_policy_clone(void* data,
                 Evas_Object* obj EINA_UNUSED,
                 void* event_info EINA_UNUSED)
{
  E_Config_Dialog_Data* cfdata = data;

  if (cfdata->policy == E_RANDR2_POLICY_CLONE)
    return;
...
Evas_Object*
advanced_settings_create(Evas_Object* win)
{
  E_Config_Dialog_Data* cfdata;
  ...
  evas_object_show(o);
  evas_object_event_callback_add(o, EVAS_CALLBACK_DEL, _del_event, NULL);
  cfdata->obj = bx = o;
  evas_object_data_set(o, "__screen_settings", cfdata);
  ...
  EINA_LIST_FOREACH(e_randr2->screens, l, s)
  {
    ...
    if (s->info.connected) {
      ...
      if (cs) {
        ...
        it = elm_hoversel_item_add(
          o, s->info.name, NULL, ELM_ICON_NONE, _cb_screen_select, cfdata);
        if (cfdata->params) {
          if ((s->info.name) && (!strcmp(s->info.name, cfdata->params)) &&
              (!first)) {
            first = s;
            first_cfg = cs;
            cfdata->screen = i;
  ...
  elm_hoversel_item_add(
    o, _("None"), NULL, ELM_ICON_NONE, _cb_rel_mode_none, cfdata);
  elm_hoversel_item_add(
    o, _("Clone"), NULL, ELM_ICON_NONE, _cb_rel_mode_clone, cfdata);
  elm_hoversel_item_add(
    o, _("Left of"), NULL, ELM_ICON_NONE, _cb_rel_mode_left_of, cfdata);
  elm_hoversel_item_add(
    o, _("Right of"), NULL, ELM_ICON_NONE, _cb_rel_mode_right_of, cfdata);
  elm_hoversel_item_add(
    o, _("Above"), NULL, ELM_ICON_NONE, _cb_rel_mode_above, cfdata);
  elm_hoversel_item_add(
    o, _("Below"), NULL, ELM_ICON_NONE, _cb_rel_mode_below, cfdata);
  ...
  EINA_LIST_FOREACH(e_randr2->screens, l, s)
  {
    ...
    if (s->info.connected) {
      it = elm_hoversel_item_add(
        o, s->info.name, NULL, ELM_ICON_NONE, _cb_rel_to_set, cfdata);
  ...
  evas_object_smart_callback_add(
    o, "changed", _cb_profile_enabled_changed, cfdata);
  ...
  evas_object_smart_callback_add(
    o, "changed", _cb_custom_scale_changed, cfdata);
  ...
  _policy_text_update(cfdata);
  elm_hoversel_item_add(
    o, _("Clone"), NULL, ELM_ICON_NONE, _cb_policy_clone, cfdata);
  elm_hoversel_item_add(
    o, _("Extend"), NULL, ELM_ICON_NONE, _cb_policy_extend, cfdata);
  elm_hoversel_item_add(
    o, _("Ask"), NULL, ELM_ICON_NONE, _cb_policy_ask, cfdata);
  elm_hoversel_item_add(
    o, _("Ignore"), NULL, ELM_ICON_NONE, _cb_policy_ignore, cfdata);
  elm_box_pack_end(bx2, o);
  evas_object_show(o);
...
void
advanced_settings_apply(Evas_Object* obj)
{
  SCOPE_DATA(obj)
  Eina_List* l;
  ...
  EINA_LIST_FOREACH(cfdata->screens, l, cs2)
  {
    ...
    cs->rel_to = NULL;
    if (cs2->rel_to)
      cs->rel_to = eina_stringshare_add(cs2->rel_to);
    ...
    cs->profile = NULL;
    if (cs2->profile)
      cs->profile = eina_stringshare_add(cs2->profile);
